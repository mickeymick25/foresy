...
RSpec.describe CraServices::Create do
  ...
  describe '.call' do
    ...
    describe 'relation-driven feature flag (DDD/RDD Compliant)' do
      # Tests for relation-driven architecture when USE_USER_RELATIONS flag is ON
      # These tests verify that UserCra pivot table is used instead of legacy created_by_user_id
      ...
      before do
        # Ensure user has independent company permissions for creation
        create(:user_company, user: user, company: independent_company, role: 'independent')
      end

      context 'when feature flag USE_USER_RELATIONS is ON' do
        before do
          # Stub the feature flag to return true
          allow(FeatureFlags).to receive(:relation_driven?).and_return(true)
        end

        it 'creates a UserCra record with creator role when flag is ON' do
          # This test should FAIL before implementation - verifies new relation-driven behavior
          result = described_class.call(
            cra_params: valid_cra_params,
            current_user: user
          )
          ...
          cra = result.data[:cra]
          expect(cra.user_cras).to be_present
          expect(cra.user_cras.creators).to be_present
          expect(cra.user_cras.creators.first.user_id).to eq(user.id)
          expect(cra.user_cras.creators.first.role).to eq('creator')
        end

        it 'still populates created_by_user_id for legacy compatibility' do
          # Keep legacy column populated while feature flag is ON
          # This ensures backward compatibility until migration to Release 3
          result = described_class.call(
            cra_params: valid_cra_params,
            current_user: user
          )
        ...
        it 'creates only one creator UserCra per CRA' do
          result = described_class.call(
            cra_params: valid_cra_params,
            current_user: user
          )
          ...
          cra = result.data[:cra]
          expect(cra.user_cras.creators.count).to eq(1)
        end

        it 'associates the correct user through UserCra' do
          result = described_class.call(
            cra_params: valid_cra_params,
            current_user: user
          )
      ...
      context 'when feature flag USE_USER_RELATIONS is OFF (legacy mode)' do
        before do
          # Stub the feature flag to return false - legacy behavior
          allow(FeatureFlags).to receive(:relation_driven?).and_return(false)
        end

        it 'does NOT create UserCra record when flag is OFF (legacy mode)' do
          result = described_class.call(
            cra_params: valid_cra_params,
            current_user: user
          )
          ...
          cra = result.data[:cra]
          # Legacy mode: no UserCra created via service
          expect(cra.user_cras.count).to eq(0)
        end
