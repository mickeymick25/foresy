...
RSpec.describe 'Authentication - Revoke', type: :request do
  ...
  path '/api/v1/auth/revoke' do
    delete 'Revoke current session token' do
      tags 'Authentication'
      ...
      response '200', 'Token revoked successfully' do
        let(:Authorization) { "Bearer #{user_token}" }
      ...
      response '401', 'unauthorized - no active session' do
        # Test when there's no active session to revoke
