...
RSpec.describe 'CRA Entries - Update', type: :request do
    ...
    patch 'Updates a CRA entry' do
      ...
      response '200', 'CRA entry updated successfully' do
        ...
        let(:entry) do
          {
            date: '2026-01-20',
            quantity: 0.5,
            description: 'Updated: Meeting with client'
          }
        end
        ...
        run_test! do |response|
          ...
          expect(data['date']).to eq('2026-01-20')
          expect(data['quantity']).to eq(0.5)
        end
      end
