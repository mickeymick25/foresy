...
# W1-D3 — Caractérisation des chemins vivants de la chaîne 429 (plan :
# docs/technical/testing/[DONE]_2026_09_18_p6_wave1_tracker.md §3 W1-D3) —
# migrée R-4 (contrat FC-05 v1, 25/09) : le limiter parallèle
# `Common::RedisRateLimiter` a été supprimé au profit de `RateLimitService`.
#
# Chaîne caractérisée (inchangée côté contrôleur) :
#   CrasController#check_rate_limit!          → RateLimitService.check_rate_limit
#                                             → CrasController#handle_rate_limit_exceeded
#                                             → error_too_many_requests → 429 + details { resource_type: 'CRA' }
#   CraEntriesController#check_rate_limit!    → RateLimitService.check_rate_limit
#                                             → CraEntriesController#handle_rate_limit_exceeded
#                                             → error_too_many_requests → 429 (sans details)
#
# Le stub fixe uniquement la réponse du service ([false, 3600] — dépassement simulé)
...
RSpec.describe 'CRA rate limit error contract', type: :request do
  ...
  before do
    # Le stub fixe uniquement la réponse du service ; le flux contrôleur
    # (auth → set_cra → check_rate_limit! → handler) s'exécute intégralement.
    allow(RateLimitService).to receive(:check_rate_limit).and_return([false, 3600])
  end
