...
RSpec.describe 'CRAs - Lock', type: :request do
  ...
  path '/api/v1/cras/{id}/lock' do
    post 'Locks a CRA to prevent modifications' do
      ...
      response '200', 'CRA locked successfully' do
        let(:id) { cra.id }
        ...
        run_test! do |response|
          ...
          expect(data['locked_at']).to be_present
        end
      end
      ...
      response '409', 'conflict - CRA already locked' do
        let(:id) { cra.id }
