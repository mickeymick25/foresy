...
| 23 | **FC-05 — Remediation contractuelle (OPEN)** — contrat cible **v1 arbitré** (A1-A9) + **RED écrits et mesurés 25/09** : `spec/services/rate_limit_service_contract_spec.rb` — **12 exemples, 10 échecs documentés** (C-1/C-2/C-3 + sémantique d'erreur + fenêtres par endpoint), 2 verts figés ; contract : `docs/technical/guides/2026_09_25_fc05_rate_limiting_contract.md` (A1-A9, 6 états de sélection, seuils contractuels, invariants R-4) — **GREEN en attente GO CTO** : implémentation minimale → GREEN → suppression du limiter parallèle → régression permanente ; vérifications Render (REDIS_URL injectée + nb d'instances) avant certification | 🔴 | contrat FC-05 §1-§7 · audit `[DONE]_2026_09_25_fc05_rate_limiting_audit.md` §10bis · étude services §4 (C-5) · message CTO 25/09 |
