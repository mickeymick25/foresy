...
| 16 | **Postman FC-08** (audit PM-1/PM-2/PM-3) — 10 endpoints manquants (Companies ×5, UserCompanies ×5), description obsolète (« 28 endpoints » vs 36 réels), variables `company_id`/`user_company_id` absentes — collection figée au 19/08, avant FC-08. **Priorisation CTO 24/09 : 🔴 2** — peut être préparé en parallèle, exécution **après qualification de #20** | 🔴 | audit `[2026_09_24_swagger_postman_api_exposure_audit.md](technical/audits/2026_09_24_swagger_postman_api_exposure_audit.md)` §4bis/§6 (P1) · message CTO 24/09 |
| 17 | **Swagger export CSV** (audit S-1) — `GET /api/v1/cras/{id}/export` public mais absent de `swagger.yaml`, exclu de l'audit CI via `EXCLUDED_ENDPOINTS` — documenter le path (200 `text/csv` + codes d'erreur FC-07) et retirer l'exclusion, **ou arbitrage CTO** de conserver l'exclusion. **Priorisation CTO 24/09 : arbitrage après vérification du contrat FC-07** | 🟠 | audit `technical/audits/2026_09_24_swagger_postman_api_exposure_audit.md` §3/§6 (P2) · message CTO 24/09 |
...
| 23 | **FC-05 — correction via RED contractuels** (GO CTO 25/09) — R-4 = architecture cible (1 mécanisme, 1 contrat, 1 chaîne de test), **implémentation NO GO avant les RED** : RED 1 sélection backend (REDIS_URL présente → `RedisBackend` ; Redis indisponible → contrat explicitement défini) · RED 2 compteur (limite 1 → req 1 ok, req 2 refusée) · RED 3 missions (limite métier **à arbitrer avant écriture**) · RED 4 CRA/entries (opération/clé/fenêtre/limite/sémantique à définir) · fail-closed par classe d'erreur (CannotConnect / Timeout / interne — jamais de 429 silencieux sur erreur interne) · puis implémentation minimale → GREEN → régression permanente ; vérifications Render (REDIS_URL injectée + nb d'instances) avant certification | 🔴 | audit `[DONE]_2026_09_25_fc05_rate_limiting_audit.md` §10bis/§10 · étude services §4 (C-5) · message CTO 25/09 |
