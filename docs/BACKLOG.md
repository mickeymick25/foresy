...
| 16 | **`Cra#validate_uniqueness` inerte à la création** : à la création, le pivot `user_cras` (rôle créateur) n'existe pas encore (créé post-insert par la factory/le flux) → `creator_user_id` nil → le garde Ruby ne s'exerce pas ; il ne protège qu'à la re-validation/update. Comportement démontré et documenté (W3-D4). **Point d'architecture** : le nom du garde laisse croire qu'il protège la création ; traiter séparément (contrainte DB ou recalage de l'ordre pivot/validation) | 🟡 WARNING / dette architecture | revue CTO PR #40 · tracker Wave 3 |
