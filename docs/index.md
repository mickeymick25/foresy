...
2. **[📁 Suivi par Phase](./technical/Done_[Done]_remediation/README.md)** - Navigation vers les 7 sous-documents de suivi détaillé (P0 à P6)
3. **[🔴 Phase 0 — Sécurité Critique](./technical/Done_[Done]_remediation/phase-0-securite.md)** - Routes test, `puts` JWT, fuite erreurs OAuth
4. **[🔴 Phase 1 — Stabilisation Runtime](./technical/Done_[Done]_remediation/phase-1-stabilisation.md)** - Crash `Domain::CraEntry`, conflit `rescue_from`
5. **[🟡 Phase 2 — Unification Erreurs](./technical/Done_[Done]_remediation/phase-2-unification-erreurs.md)** - Phase 1.9 + suppression `ErrorRenderable` + `render_fc07_error`
6. **[🟡 Phase 3 — Nettoyage Code Mort](./technical/Done_[Done]_remediation/phase-3-nettoyage-code-mort.md)** - ~2700 lignes `app/lib` + concerns orphelins
7. **[🟡 Phase 4 — Cohérence Architecturale](./technical/Done_[Done]_remediation/phase-4-coherence-architecturale.md)** - Héritage contrôleurs, IP rate limit, services missions
8. **[🟢 Phase 5 — DB & Config](./technical/Done_[Done]_remediation/phase-5-db-config.md)** - UUID natif, enum PG, module `Foresy`, `load_defaults`
9. **[🟢 Phase 6 — Hardening Final](./technical/Done_[Done]_remediation/phase-6-hardening-final.md)** - GitLedger shell, `CraEntry` nettoyage, `users` PK
