...
| D-2 | Couverture de lignes (SimpleCov) | **Chiffrage produit le 16/09** : `docs/technical/changes/2026-09-16-D2_SimpleCov_Chiffrage.md` — ~3 h effectives (½ journée) : gems `simplecov` + `simplecov-cobertura`, `.simplecov` (lignes + branche, seuil 95% standard maison), plomberie CI déjà en place (COVERAGE=true + upload artefact) ; 3 décisions CTO en attente (seuil initial, Cobertura, sprint) | 🟡 Moyenne | Transverse |
...
| D-11 | Dépréciation Node.js 20 dans les actions GitHub — **5 warnings CI/CD** | **RÉSOLUE le 16/09** (branche `chore/d2-d11-quality-tooling`) : bump `actions/checkout` v4→v5 (×5) et `actions/upload-artifact` v4→v5 (×3) dans `.github/workflows/ci.yml` — versions Node 24 ; YAML validé, gate documentée : run CI vert sur la PR avant merge | ✅ Résolue (PR en cours) | CI/CD transverse |
...
6. **D-11** (hors FC-08) — ✅ bump posé le 16/09 (branche `chore/d2-d11-quality-tooling` : `checkout@v5` ×5 + `upload-artifact@v5` ×3) — merge après run CI vert
