...
**Périmètre :** toute la dette du registre `docs/technical/2026_09_14_fc08_debt_register.md`, depuis D-1 (D-4 incluse)
...
| **D-8** Signup 400 | Fix vérifié : `StandardError` déclaré **en premier** dans `StandardizedError` (mécanisme `rescuable.rb` commenté dans le code) ; spec contractuelle présente dans `users_spec.rb` (`signup {}` → 400 `MISSING_PARAMETER`, forme plate, renvoi au contrat `2026_08_18_error_contract.md`) — TDD RED→GREEN documenté ; contrat p1_2 (générique → 500) préservé ; **smoke 15/15 exécuté** ; discipline « RAG d'abord » tracée au journal | ✅ Conforme, platinium |
| **D-9** rubyzip CVE | `Gemfile.lock` : rubyzip 3.6.0 ; **`bundler-audit check --update` : 0 vulnérabilité** (advisory DB 2026-09-13) | ✅ Conforme |
| **D-10** Isolement bases | Guide `docs/technical/testing/2026_09_15_test_database_isolation.md` (piège `DATABASE_URL`, procédure conteneur, validation) ; suite exécutée sur `foresy_test` via la config compose | ✅ Conforme, platinium |
...
- Journal : `docs/technical/[DONE]_2026_08_31_fc08_implementation_tracker.md` (entrées Gate CTO, CI Gate, D-4, D-5, D-8, D-9, D-10)
