...
- **Les 9 fichiers :** 7 services legacy `Api::V1::*` (`app/services/api/v1/cras/*` ×6 + `cra_entries/list_service.rb`) — le plan `legacy_cleanup_plan.md` (07/01, jamais exécuté) les listait comme « jamais utilisés » depuis la migration FC-07 vers `CraServices::*` — plus `app/lib/http_status_map.rb` et `app/lib/mission_errors.rb` (zéro référence active). Reproduction locale parfaite : `CI=1` → **67,27 % identiques à la décimale près**
- **Cleanup exécuté (GO CTO 18/09, extension vérifiée incluse) :** suppression des 9 fichiers ; plan marqué exécuté (relevé en fin de document). Validation : suite **962/0 inchangée**, RuboCop **0**, Brakeman **0**, grep résiduel = commentaires de migration uniquement. Corpus CI-sim : 67,27 % → **72,34 %** (2932/4053) ; lazy inchangé **73,21 %**
