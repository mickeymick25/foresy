...
- **Fichiers modifiés :** docs/technical/[DONE]_2026_08_31_fc08_implementation_tracker.md
- **Tests :** N/A
...
- **Description :** `docs/technical/changes/[DONE]_2026_09_14_FC08_Companies_PR_Description.md` (résumé, evidence de tests, 18 commits, DoD 9/9, points d'attention revue)
...
- **RAG d'abord** : le plan D-8 a été corrigé après requête hub (`guides/2026_08_18_error_contract.md` = contrat officiel ParameterMissing → 400 ; P1.2 = historique du bloc temporaire, piste distincte)
...
- **Documentation :** `docs/technical/testing/2026_09_15_test_database_isolation.md`
...
- **Les 9 fichiers :** 7 services legacy `Api::V1::*` (`app/services/api/v1/cras/*` ×6 + `cra_entries/list_service.rb`) — le plan `docs/technical/changes/[DONE]_2026_01_07_Legacy_Cleanup_Plan.md` (07/01, jamais exécuté — régularisé depuis la racine le 18/09) les listait comme « jamais utilisés » depuis la migration FC-07 vers `CraServices::*` — plus `app/lib/http_status_map.rb` et `app/lib/mission_errors.rb` (zéro référence active). Reproduction locale parfaite : `CI=1` → **67,27 % identiques à la décimale près**
- **Cleanup exécuté (GO CTO 18/09, extension vérifiée incluse) :** suppression des 9 fichiers ; plan marqué exécuté (relevé en fin de document, régularisé dans `docs/technical/changes/[DONE]_2026_01_07_Legacy_Cleanup_Plan.md`). Validation : suite **962/0 inchangée**, RuboCop **0**, Brakeman **0**, grep résiduel = commentaires de migration uniquement. Corpus CI-sim : 67,27 % → **72,34 %** (2932/4053) ; lazy inchangé **73,21 %**
...
- **Rapport complet :** `docs/technical/changes/[DONE]_2026_09_16_D1-D11_Debt_Verification_Report.md`
